    ...
    render() {
        ...
        return (
            <SafeAreaView style={{ flex: 1 }}>
                <View style={{ flex: 1, backgroundColor: "#2c2c39", }} onTouchStart={() => {this.HideMenu()}}>
                    ...
                    <View style={{ width: Scales.deviceWidth * 1.0, }}>
                        ...
                        <View style={{ width: Scales.deviceWidth * 1.0, height: Scales.deviceHeight * 0.06, flexDirection: "row", justifyContent: "center", alignItems:'flex-end', paddingBottom:0}}>
                            <View style={{ width: Scales.deviceWidth * 0.8, height: Scales.deviceHeight * 0.06, borderRadius: 5, alignSelf: 'center' }}>
                                <TextInput onChangeText={(text) => this.searchJobFilterFunction(text)} value={this.state.seleted_job_name} style={{ backgroundColor: '#3d3d46', width: Scales.deviceWidth * 0.8, paddingLeft: Scales.deviceWidth * 0.08, fontFamily: "roboto-medium", height: Scales.deviceHeight * 0.06, borderRadius: 5, borderBottomRightRadius: 0, borderTopRightRadius: 0, borderRightWidth: 0, borderWidth: 0.5, fontSize: Scales.moderateScale(14), color: "white" }} />
                        ...
                        {this.state.jobs_show ? <View style={{ width: Scales.deviceWidth * 0.92, height: Scales.deviceHeight * 0.30, backgroundColor: "#52526c", borderBottomLeftRadius: 10, borderBottomRightRadius:10, alignSelf: "center", }}>
                            ...
                            {this.state.filter_jobs.length != 0 ? <View style={{ width: Scales.deviceWidth * 0.92, height: Scales.deviceHeight * 0.30, position: "absolute", }}>
                                <FlatList
                        ...
                        {this.state.jobs.length != 0 ? <View style={{ width: Scales.deviceWidth * 1.0, height: Scales.deviceHeight * 0.84, alignSelf: "center", }}>
                            <FlatList
                                ...
                                renderItem={({ item }) => <InterviewList date_format={this.state.date_format} data={item} dotmenu={this.state.dotmenu} GetTrackData={this.GetTrackData} navigation={this.props.navigation} />}
    ...
    render() {
        ...
        if (this.props.index == 0) {
            render_data = <TouchableOpacity onPress={() => this.props.OnSelecteJob(this.props.data)}><View style={{ width: Scales.deviceWidth * 0.95, height: Scales.deviceHeight * 0.05, justifyContent: "center", }}>
                <Text style={{ paddingLeft: 10, fontSize: Scales.moderateScale(14), fontFamily: "roboto-medium", color:'white' }}>{this.props.data.name}</Text>
            ...
            render_data = <TouchableOpacity style={{ zIndex: 1000 }} onPress={() => this.props.OnSelecteJob(this.props.data)}><View style={{ width: Scales.deviceWidth * 0.95, height: Scales.deviceHeight * 0.05, borderTopWidth: 0.3, justifyContent: "center", zIndex: 100 }}>
                <Text style={{ paddingLeft: 10, fontSize: Scales.moderateScale(14), fontFamily: "roboto-medium", white:'white' }}>{this.props.data.name}</Text>
...
class InterviewList extends Component {
    ...
    componentWillReceiveProps = () => {
        this.setState({dotmenu:this.props.dotmenu})
    }
    ...
    render() {
        ...
        return (
            <View style={{ width: Scales.deviceWidth * 1.0, alignSelf: "center", }} >
                {/* TO REMOVE SHADOW IF ASKED */}
                <View style={{ width: Scales.deviceWidth * 0.92, alignSelf: "center",  backgroundColor: "#3d3d46", elevation: 9, borderRadius: 5, padding: 5, paddingTop: 10, flexDirection: "row",shadowOffset: { width: 0, height: 1 },
                                shadowOpacity: 0.1,
                                shadowRadius: 1 ,
                                paddingLeft:Scales.deviceHeight * 0.02,
                                marginBottom:'2.5%',
                        }}>
                    {/* VIEW FOR CONTENT INSIDE FLATLIST ITEM */}
                    <View style={{ flexDirection: 'row' }}>
                        {/* VIEW FOT IMAGE AND STATUS */}
                        <View>
                            <View style={{ width: Scales.deviceHeight * 0.08, justifyContent: "center", height: Scales.deviceHeight * 0.08, borderRadius: Scales.deviceHeight * 0.0085, backgroundColor: this.state.image == "" ? "#2d2d3a" : "transparent", }}>
                                {this.state.image == "" ?
                                    <Text style={{ fontSize: Scales.moderateScale(22), textTransform: "capitalize", fontFamily: "roboto-medium", color: "white", alignSelf: "center" }}>{this.props.data.pitcher.pitcher_fname.slice(0, 2)}</Text> :
                                    <Image onError={() => { this.setState({ image: "" }) }} source={{ uri: this.props.data.pitcher.pitcher_url }} style={{ width: Scales.deviceHeight * 0.08, height: Scales.deviceHeight * 0.085, borderRadius: Scales.deviceHeight * 0.008 }} />}
                            </View>
                            <View style={{
                                width: Scales.deviceHeight * 0.08,
                                marginTop: 2,
                                borderRadius: 4,
                                backgroundColor: this.props.data.jobma_interview_status == 0 ? "#ffd801" :
                                    this.props.data.jobma_interview_status == 1 ? "#52526c" :
                                        this.props.data.jobma_interview_status == 2 ? "#13d7a6" :
                                            expired_flag ? "red" :
                                                this.props.data.jobma_interview_status == 3 ? "red" :
                        ...
                        {/* VIEW FOR DETAILS */}
                        <View style={{ width: Scales.deviceWidth * 0.65 }}>
                            <View style={{ alignItems: "center", flexDirection: "row", }}>
                                <Text style={{ fontFamily: "roboto-regular", fontSize: Scales.moderateScale(14), paddingLeft: 8, color: "white" }}>
                                    <Text style={{ fontFamily: "roboto-bold", color: "white" }}>
                                        {this.props.data.pitcher.pitcher_fname} {this.props.data.pitcher.pitcher_lname}
                                    </Text> Invited for
                                     <Text style={{ fontFamily: "roboto-bold" }}> {this.props.data.job.job_title}</Text>
                                </Text>
                            </View>
                            <View style={{ flexDirection: "row", padding: 3, paddingLeft: 7, paddingBottom: 0 }}>
                                <Image 
                                    source={require("../../assets/JobmaIcons/email.png")} 
                                    style={{ resizeMode: "contain", height: Scales.deviceHeight * 0.018, width: Scales.deviceWidth * 0.04, marginRight: 6 }} />
                                <Text 
                                    style={{ fontFamily: "roboto-medium", fontSize: Scales.moderateScale(12), color: "#8b8b8b" }}>
                                        {this.props.data.pitcher.pitcher_email}
                                </Text>
                            </View>
                            <View style={{ flexDirection: "row", padding: 3, paddingLeft: 7, paddingBottom: 0 }}>
                                <Image source={require("../../assets/JobmaIcons/pre-recorded.png")} style={{ resizeMode: "contain", height: Scales.deviceHeight * 0.018, width: Scales.deviceWidth * 0.04, marginRight: 6 }} />
                                <Text style={{ fontFamily: "roboto-medium", fontSize: Scales.moderateScale(12), color: "#8b8b8b" }}>{this.props.data.jobma_interview_mode == 1 ? "Pre-recorded Interview" : this.props.data.jobma_interview_mode == 0 ? 'Offline Interview' : "Live Interview"}</Text>
                            </View>
                            <View style={{ flexDirection: "row", padding: 3, paddingLeft: 7, paddingBottom: 0 }}>
                                <Image source={require("../../assets/JobmaIcons/calendar-lightBlue.png")} style={{ resizeMode: "contain", height: Scales.deviceHeight * 0.018, width: Scales.deviceWidth * 0.04, marginRight: 6 }} />
                                <Text style={{ fontFamily: "roboto-medium", fontSize: Scales.moderateScale(12), color: "#8b8b8b" }}>{moment(this.props.data.jobma_invitation_date).format(this.props.date_format)} - </Text>
                                <Text style={{ fontFamily: "roboto-medium", fontSize: Scales.moderateScale(12), color: "#8b8b8b" }}>{moment(this.props.data.jobma_invitation_expire_date).format(this.props.date_format)}</Text>
                            </View>
                        </View>
                        {/* VIEW FOR 3 DOT MENU */}
                        <View>
                        ...
                        <View style={{ borderRadius: 5, position: 'absolute', top: -1, right: 16, backgroundColor: '#52526c', width: Scales.deviceWidth * 0.3, zIndex: 100 }}>
                            <View style={{ display: this.state.dotmenu, paddingTop: '4%', paddingBottom: '4%', }}
                                onTouchStart={(e) => {
                                    e.stopPropagation()
                                }}
                            >
                                <TouchableOpacity 
                                    onPress={() => this.props.navigation.navigate("Evaluate", { "appiled_id": this.props.data.applied_id, "image": this.props.data.pitcher.pitcher_url })}
                                    style={{ paddingLeft: Scales.deviceWidth * 0.025, paddingRight: Scales.deviceWidth * 0.005, }}
                                >
                                    <Text style={{ color: 'white', fontSize: 13, padding: '5%', }}>Evaluate</Text>
                                </TouchableOpacity>
                                <TouchableOpacity 
                                    onPress={() => this.setState({ send_reminder_modal: true })}
                                    style={{ paddingLeft: Scales.deviceWidth * 0.025, paddingRight: Scales.deviceWidth * 0.005, borderTopWidth: 0.25, borderBottomWidth: 0.25, }}
                                >
                                    <Text style={{ color: 'white', fontSize: 13, padding: '5%', }}>Send Reminder</Text>
                                </TouchableOpacity>
                                <TouchableOpacity 
                                    onPress={() => this.CopiedInvitationLink()} 
                                    style={{ paddingLeft: Scales.deviceWidth * 0.025, paddingRight: Scales.deviceWidth * 0.005, }}
                                >
                                    <Text style={{ color: 'white', fontSize: 13, padding: '5%', }}>Invitation Link</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                    </View>
                    {/* <View style={{ backgroundColor:'grey',width: Scales.deviceWidth * 0.16,  height: Scales.deviceHeight * 0.12,paddingTop: Scales.deviceHeight * 0.01, }}>
                ...
                <Modal style = {{  margin: 0 }} isVisible={this.state.send_reminder_modal} onBackButtonPress={() => this.setState({ send_reminder_modal: false, selected_date: this.props.data.jobma_invitation_expire_date })}>
                    <View style={{ flex: 1, justifyContent:'flex-end', }}>
                {/* <KeyboardDoneButton style={{flex:1, borderRadius: 10,}} /> */}

                        <View style={{ width: Scales.deviceWidth * 1, height: Scales.deviceHeight * 0.60, backgroundColor: "#52526c", borderTopLeftRadius: 20, borderTopRightRadius:20,marginTop:'5%' }}>
