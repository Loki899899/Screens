...
export default class Joblisting extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            is_inactive_selected:false,
            is_expired_selected:false,
        }
    }
    ...
    render() {
        ...
        return (
            <SafeAreaView style={{ flex: 1 }}>
                <View style={{ flex: 1, }}>
                    <Header heading="Jobs Listing" {...this.props} textalign='left' widths={Scales.deviceWidth * 0.55} left={Scales.deviceWidth * 0.25} Click_Search={this.Click_Search} filter_data={this.props.filter_data} />
                    ...
                    <View style={{ flex: 1, backgroundColor: '#2d2d3a' }}>
                        ...
                        <TouchableOpacity
                                style={{ backgroundColor: this.state.is_all_selected?'#13d7a6':'#52526c', borderRadius: 20, marginLeft:'1%', }}
                                onPress={() => {
                                    this.setState({is_all_selected:true})
                                    this.setState({is_active_selected:false})
                                    this.setState({is_inactive_selected:false})
                                    this.setState({is_expired_selected:false})
                                }}
                            >
                                <Text style={{
                                    color: this.state.is_all_selected?'black':'white',
                                    fontSize: 12,
                                    padding: '3%',
                                    paddingLeft: '6%',
                                    paddingRight: '6%'
                            ...
                            <TouchableOpacity
                                style={{ backgroundColor: this.state.is_active_selected?'#13d7a6':'#52526c' , borderRadius: 20, marginLeft:'1%', }}
                                onPress={() => {
                                    this.setState({is_all_selected:false})
                                    this.setState({is_active_selected:true})
                                    this.setState({is_inactive_selected:false})
                                    this.setState({is_expired_selected:false})
                                }}
                            >
                                <Text style={{
                                    color: this.state.is_active_selected?'black':'white',
                                    fontSize: 12,
                                    padding: '3%',
                                    paddingLeft: '6%',
                                    paddingRight: '6%'
                            ...
                            <TouchableOpacity
                                style={{ backgroundColor: this.state.is_inactive_selected?'#13d7a6':'#52526c', borderRadius: 20, marginLeft:'1%', }}
                                onPress={() => {
                                    this.setState({is_all_selected:false})
                                    this.setState({is_active_selected:false})
                                    this.setState({is_inactive_selected:true})
                                    this.setState({is_expired_selected:false})
                                }}
                            >
                                <Text style={{
                                    color: this.state.is_inactive_selected?'black':'white',
                                    fontSize: 12,
                                    padding: '3%',
                                    paddingLeft: '6%',
                                    paddingRight: '6%'
                            ...
                            <TouchableOpacity
                                style={{ backgroundColor: this.state.is_expired_selected?'#13d7a6':'#52526c', borderRadius: 20, marginLeft:'1%', }}
                                onPress={() => {
                                    this.setState({is_all_selected:false})
                                    this.setState({is_active_selected:false})
                                    this.setState({is_inactive_selected:false})
                                    this.setState({is_expired_selected:true})
                                }}
                            >
                                <Text style={{
                                    color: this.state.is_expired_selected?'black':'white',
                                    fontSize: 12,
                                    padding: '3%',
                                    paddingLeft: '6%',
                                    paddingRight: '6%'
